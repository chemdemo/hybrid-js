...
* @Last Modified time: 2016-04-05 20:02:57
...
const SCHEMA = 'hybrid'; // 私有协议，建议替换成公司私有协议，如腾讯QQ“qq”
...
Hybrid.debug = false
// 重置app监听mback的状态
invokeAppApi('sdk.set_status', {key: 'interceptOnBack', value: 0})
...
function invokeAppApi(api, params) {
    ...
    if(Object.keys(params).length <= 1) for(let key in params) v = params[key]
    else v = JSON.stringify(params)
...
function on(event, handler) {
    ...
    // 如果是mback事件，则直接通知app，H5需要监听mback
    if(MBACK_EVENT_NAME === event) invokeAppApi('sdk.set_status', {key: 'interceptOnBack', value: 1})
...
function off(event, handler) {
    ...
    if(MBACK_EVENT_NAME === event) invokeAppApi('sdk.set_status', {key: 'interceptOnBack', value: 0})
